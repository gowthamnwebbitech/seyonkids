...
@section('admin')
    ...
    <div class="card-header">
        ...
        <button type="button" class="btn btn-primary"
            onclick="window.location.href = '{{ route('admin.product.subcategory.add') }}'">
            Add New
        </button>
                ...
                <tbody>
                    @foreach ($category as $key => $item)
                        <tr>
                            ...
                            <td>
                                @php
                                    $subcategory = App\Models\ProductCategory::find($item->category_id);
                                @endphp
                                {{ $subcategory->name ?? '' }}
                            </td>
                            <td>{{ $item->name }}</td>
                            <td>
                                <img src="{{ asset($item->subcategory_image) }}" style="width:60px" alt="Icon Image">
                            </td>
                            <td>
                                @if ($item->status == 1)
                            ...
                            <td>
                                <div class="d-flex">
                                    <a href="{{ route('admin.product.subcategory.edit', $item->id) }}"
                                        class="btn btn-success btn-xs me-2">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <a href="{{ route('product.subcategory.delete', $item->id) }}"
