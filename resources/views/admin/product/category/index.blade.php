...
@section('admin')
    ...
    <div class="card-header">
        ...
        <button type="button" class="btn btn-primary"
            onclick="window.location.href = '{{ route('admin.product.category.add') }}'">
            Add New
        </button>
                ...
                <tbody>
                    @foreach ($category as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->name }}</td>
                            <td>
                                <img src="{{ asset($item->category_image) }}" style="width:60px" alt="Icon Image">
                            </td>
                            <td>
                                @if ($item->status == 1)
